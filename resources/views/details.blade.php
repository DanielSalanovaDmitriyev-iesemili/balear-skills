...
<body>
    @include('layouts.header')
        <main>
            ...
            <div>
            <img src='{{asset('img/' . $space->img . $space->id . '.png')}}' class='h-64 md:h-80 rounded-lg bg-gray-100 mb-4'>
            ...
            <div class='md:flex-1 px-4'>
            <h2 class='mb-2 leading-tight tracking-tight font-bold text-gray-800 text-2xl md:text-3xl'>{{$space->name}}</h2>
            ...
            <div class="ml-96">
                @if (Auth::check())
                <div class="flex">
                    <form action="{{route('comment', $space->id)}}" method="POST">
                        @csrf
                        <h1>Comenta!</h1>
                        <textarea name="" id="" cols="30" rows="5" name="comment" required  minlength="3" maxlength="150"></textarea>
                ...
                <div>
                    <h1>Comentarios</h1>
                    @foreach ($space->user()->get() as $comment)
                    <div class="flex"> 
                        <h1>Usuario: {{$comment->name}}</h1>
                        <p></p>
